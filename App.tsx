...
const App: React.FC = () => {
  ...
  // Check for API Key on mount (visual warning only)
  useEffect(() => {
...
generated by Chart Wargame
  ...
  const handleImageSelect = async (base64: string) => {
    ...
    try {
      const analysisResult = await analyzeChart(base64);
      setAnalysis(analysisResult);
      
      const detectedTicker = analysisResult.ticker || '';
      setEditableTicker(detectedTicker);
      
      // If we found a ticker, auto-fetch market context
      if (detectedTicker) {
        setAppState(AppState.FETCHING_DATA);
        try {
          // Pass technical summary to prioritize relevant news
          const news = await fetchMarketContext(detectedTicker, analysisResult.technical_summary);
          setMarketData(news);
        } catch (e) {
      ...
      if (!process.env.API_KEY) {
        setError("SYSTEM FAILURE: API_KEY MISSING.");
      } else {
        setError("OBSERVER FAILED. COULD NOT ANALYZE CHART.");
      }
    }
  };
  ...
  const handleManualIntelRefresh = async () => {
    ...
    try {
      const news = await fetchMarketContext(editableTicker, analysis.technical_summary);
      setMarketData(news);
      setAppState(AppState.CHART_READY);
    } catch (err) {
      console.error(err);
      setError("SCOUT FAILED TO FETCH NEWS.");
      setAppState(AppState.CHART_READY);
    }
  };
  ...
  const handleSimulate = async (scenario: string) => {
    ...
    try {
      ...
      const simResult = await runSimulation(image, analysis, scenario, marketData || undefined, editableTicker);
      setSimulation(simResult);
      setAppState(AppState.SIMULATED);
    } catch (err) {
      console.error(err);
      setAppState(AppState.ERROR);
      setError("SIMULATION FAILED. ORACLE DISCONNECTED.");
    }
  };
      ...
      <header className="h-16 border-b border-gray-800 flex items-center justify-between px-6 bg-black/50 backdrop-blur-sm sticky top-0 z-50">
        ...
        <div className="flex items-center gap-6">
           ...
           <div className="hidden md:flex items-center gap-4 text-xs font-mono text-gray-500 border-l border-gray-800 pl-6">
              ...
              <div className="flex items-center gap-1">
                <span className={`w-2 h-2 rounded-full ${isWorking ? 'bg-yellow-500 animate-ping' : 'bg-radar-green'}`}></span>
                     ...
                     <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="ml-2 flex items-center gap-2">
                           ...
                           <button 
                             ...
                             title="Refresh Market Intelligence for this Ticker"
                           >
                             <RefreshCw size={12} />
                           </button>
                        </div>
