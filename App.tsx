...
const App: React.FC = () => {
  ...
  // Default to the first one in the pipeline
  const [activeModel, setActiveModel] = useState<string>(MODEL_PIPELINE[0]);
  ...
  const handleShareIntel = async () => {
    // We need at least analysis to share
...
------------------------------
📉 TICKER: ${editableTicker || analysis.ticker || "UNKNOWN"}
...
`.trim();
    ...
    try {
      ...
      try {
        ...
        // Continue to image generation even if clipboard fails
      }
  ...
  // If activeModel is NOT the first one in the pipeline, we consider it "Fallback"
  const isFallbackModel = activeModel !== MODEL_PIPELINE[0];
      ...
      <main className="flex-1 p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-[1600px] mx-auto w-full">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6" ref={visualizationRef}>
          ...
          <div className="bg-tactical-gray border border-gray-700 rounded-lg p-4 min-h-[180px] flex flex-col relative overflow-hidden group">
             ...
             {/* Share Button (Visible when ANALYSIS OR simulation ready) */}
             {((analysis || simulation) && !isWorking) && (
                <button
             ...
             {appState === AppState.ERROR && (
                <div className="flex items-start gap-3 text-red-500 font-mono bg-red-950/20 p-2 rounded border border-red-900 overflow-auto max-h-40">
